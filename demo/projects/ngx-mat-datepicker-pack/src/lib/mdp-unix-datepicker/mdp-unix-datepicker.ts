...
@Component({
  selector: 'mdp-unix-datepicker',
  imports: [ReactiveFormsModule],
  templateUrl: './mdp-unix-datepicker.html',
  styleUrl: './mdp-unix-datepicker.scss',
  providers: [
    {
      provide: MatFormFieldControl,
      useExisting: MdpUnixDatepicker
    },
  ],
})
export class MdpUnixDatepicker implements ControlValueAccessor, MatFormFieldControl<Date>, OnDestroy, OnInit {
  ...
  static nextId = 0;
  readonly id = `mdp-unix-datepicker-${MdpUnixDatepicker.nextId++}`;

  readonly ngControl = inject(NgControl, { optional: true, self: true });
  readonly controlType = 'mdp-unix-datepicker';
